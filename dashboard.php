...
<header class="bg-white">
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
            <span class="text-sm font-semibold leading-6 text-gray-900"><?php echo $_SESSION['user']?></span> &nbsp;&nbsp;
            <a class="text-sm font-semibold leading-6 text-gray-900" href="logout.php">Log Out</a>
        </div>
    </nav>
        ...
        <div class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div class="mt-6 flow-root">
                <div class="-my-6 divide-y divide-gray-500/10">
                    <div class="py-6">
                        <span class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"><?php echo $_SESSION['user']?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="relative flex min-h-screen overflow-hidden bg-gray-50 py-6 sm:py-12">
        ...
        <div class="relative max-w-7xl mx-auto">
            <div class="flex justify-end">
                <span class="block text-gray-600 font-mono border border-gray-400 rounded-xl px-2 py-1">Your feedback form link: <strong>http://localhost/anonymous-feedback-app/feedback.php?user=<?php echo $_SESSION['link_id']?></strong></span>
            </div>
            <h1 class="text-xl text-indigo-800 text-bold my-10">Received feedback</h1>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <?php
                    foreach ($feedback_data as $feedback) {
                        if($_SESSION['link_id'] == $feedback['link_id']){ ?>
